...
│   └── database/supabase.service.ts   # cliente Supabase server-side (Secret Key)
...
**RLS**: habilitado em `usuarios`/`auditoria_app`/`convites`, funções `current_empresa()`/`current_papel()` (SECURITY DEFINER, `search_path` vazio, EXECUTE restrito à role `authenticated`) para políticas `id_empresa = current_empresa() OR current_papel() = 'platform_admin'`. Na prática, o NestJS usa a Secret Key do Supabase (novo sistema de API Keys — `sb_secret_...`, substitui a chave legada `service_role` neste projeto novo) para escritas em nome de requisições já autorizadas — RLS é a camada de defesa extra, não o mecanismo primário de autorização (que é o `RolesGuard` do NestJS + escopo `id_empresa` explícito em toda chamada ao gateway).
